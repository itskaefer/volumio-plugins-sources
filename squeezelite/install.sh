...
if [ ! -f $INSTALLING ]; then
  ...
  if [ ! -d /opt/squeezelite ]; then
    ...
    if [ $dist = "buster" ] && [ $variant = "minidspshd" ] && ( [ $arch = "armv6l" ] || [ $arch = "armv7l" ] || [ $arch = "aarch64" ] ); then
      echo "Using squeezelite 1.9.9 armel static architecture (detected minidsp SHD)"
      ln -fs $PLUGIN_DIR/known_working_versions/squeezelite-arm-minidspshd /opt/squeezelite
